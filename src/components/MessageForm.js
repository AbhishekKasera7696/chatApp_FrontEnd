...
//     const { currentRoom, socket, setMessages, messages, privateMemberMsg} = useContext(AppContext);
...
const MessageForm = () => {
    ...
    socket.emit('room-messages').on('room-messages', (roomMessages) => {
        console.log('room-messages', roomMessages);
        setMessages(roomMessages)
    });
            ...
            {user && privateMemberMsg?._id && (
                <>
                    <div className='alert alert-info conversation-info'>
                        <div>
                            Your conversation with {privateMemberMsg.name} <img src={privateMemberMsg.picture} className='conversation-profile-picture' alt='img'/>
                        </div>
                    </div>
                </>
            )}
                        ...
                        <div key={msgIdx} className={sender?.email === user?.email ? "message" : "incoming-message"}>
                            <div className='message-inner'>
                                <div className='d-flex align-items-center mb-3'>
                                    <img src={sender.picture} style={{width: 35, height: 35, objectFit: "cover", borderRadius: "50%", marginRight: 10}} alt="pic"/>
