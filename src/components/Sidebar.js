...
const Sidebar = () => {
    ...
    function getRooms() {
      fetch('https://full-stack-appchat.herokuapp.com/rooms' || 'http://localhost:5001/rooms').then((res) => res.json()).then((data) => setRooms(data))
    }
        ...
        <ListGroup>
            {rooms.map((room,idx) => (
                  <ListGroup.Item key={idx} onClick={() => joinRoom(room)} active={room === currentRoom} style={{cursor: "pointer", display: "flex", justifyContent: "space-between"}}>
                    {room} {currentRoom !== room && <span className='badge rounded-pill bg-primary'>
